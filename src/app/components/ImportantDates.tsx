...
export function ImportantDates() {
  ...
  const dates = [
    { icon: Calendar, label: t('date_submission'), date: '30 November 2026', color: 'from-[#F2B21A] to-[#2CA6C4]' },
    { icon: Clock, label: t('date_early'), date: '15 December 2026', color: 'from-[#2CA6C4] to-[#35B0B7]' },
    { icon: MapPin, label: t('date_conference'), date: '', color: 'from-[#1E73A8] to-[#2CA6C4]' }
  ];
            ...
            {dates.map((item, index) => (
              <div key={index} className="relative flex items-center">
                ...
                <div className={`${index % 2 === 0 ? 'pr-1/2 rtl:pl-1/2 rtl:pr-0' : 'pl-1/2 rtl:pr-1/2 rtl:pl-0'} w-1/2 ${index % 2 === 0 ? 'text-right rtl:text-left' : 'text-left rtl:text-right ml-auto rtl:mr-auto rtl:ml-0'}`}>
                  <div className="p-6 rounded-2xl bg-card border border-border hover:border-primary/50 hover:shadow-xl hover:shadow-primary/10 transition-all duration-300">
